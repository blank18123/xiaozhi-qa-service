...
class ASRProvider(ASRProviderBase):
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            ...
            logger.bind(tag=TAG).info("ASR WebSocket连接已建�?)
            self.server_ready = False
            ...
            if conn.asr_audio and len(conn.asr_audio) > 0:
                ...
                logger.bind(tag=TAG).info("已发送首帧，开始识�?)
        ...
        try:
            ...
            if self.asr_ws and self.is_processing:
                try:
                    ...
                    logger.bind(tag=TAG).debug(f"已发送停止请�?)
    ...
    async def _send_stop_request(self):
        ...
        if self.asr_ws:
            try:
                ...
                self.is_processing = False
                await self._send_audio_frame(b"", STATUS_LAST_FRAME)
                logger.bind(tag=TAG).debug("已发送停止请�?)
            except Exception as e:
                logger.bind(tag=TAG).error(f"发送停止请求失�? {e}")
    ...
    async def close(self):
        ...
        if hasattr(self, "decoder") and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(f"释放Xunfei decoder资源时出�? {e}")
