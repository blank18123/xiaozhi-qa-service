...
class ASRProviderBase(ABC):
    ...
    # 鎵撳紑闊抽閫氶亾
        ...
        """灏哖CM鏁版嵁杞崲涓篧AV鏍煎紡"""
        if len(pcm_data) == 0:
            logger.bind(tag=TAG).warning("PCM鏁版嵁涓虹┖锛屾棤娉曡浆鎹AV")
            return b""

        # 纭繚鏁版嵁闀垮害鏄伓鏁帮紙16浣嶉煶棰戯級
        if len(pcm_data) % 2 != 0:
            pcm_data = pcm_data[:-1]
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"WAV杞崲澶辫触: {e}")
            return b""
    ...
    def build_temp_file(self, pcm_bytes: bytes) -> Optional[str]:
        try:
            ...
            logger.bind(tag=TAG).error(f"涓存椂闊抽鏂囦欢鐢熸垚澶辫触: {e}")
            return None
    ...
    async def speech_to_text_wrapper(
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"璇煶璇嗗埆澶辫触: {e}")
            return None, None
        finally:
            try:
    ...
    async def speech_to_text(
        ...
        :param opus_data: 杈撳叆鐨凮pus闊抽鏁版嵁
        :param session_id: 浼氳瘽ID
        :param audio_format: 闊抽鏍煎紡锛岄粯璁?opus"
        :param artifacts: 闊抽宸ヤ欢锛屽寘鍚玃CM鏁版嵁銆佹枃浠惰矾寰勭瓑
    ...
    def decode_opus(opus_data: List[bytes]) -> List[bytes]:
        """灏哋pus闊抽鏁版嵁瑙ｇ爜涓篜CM鏁版嵁"""
        decoder = None
        try:
            decoder = opuslib_next.Decoder(16000, 1)
            pcm_data = []
            buffer_size = 960  # 姣忔澶勭悊960涓噰鏍风偣 (60ms at 16kHz)
            ...
            for i, opus_packet in enumerate(opus_data):
                try:
                    ...
                    logger.bind(tag=TAG).warning(f"Opus瑙ｇ爜閿欒锛岃烦杩囨暟鎹寘 {i}: {e}")
                except Exception as e:
                    logger.bind(tag=TAG).error(f"闊抽澶勭悊閿欒锛屾暟鎹寘 {i}: {e}")

            return pcm_data

        except Exception as e:
            logger.bind(tag=TAG).error(f"闊抽瑙ｇ爜杩囩▼鍙戠敓閿欒: {e}")
            return []
        finally:
