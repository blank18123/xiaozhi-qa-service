...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        # sentence_id 鍒版枃鏈殑鏄犲皠锛岀敤浜庢祦寮廡TS鑾峰彇姝ｇ‘鐨勫瓧骞曟枃鏈?
        self._sentence_text_map = {}
        # 鍔犺浇鏇挎崲璇嶏紝鐢ㄤ簬涓€娆℃€ф鍒欐浛鎹?
        ...
        if self.correct_words:
            ...
            # 鏋勫缓鍙嶅悜鏇挎崲姝ｅ垯锛岀敤浜庡皢TTS鏈嶅姟杩斿洖鐨勬浛鎹㈠悗鏂囨湰杩樺師涓哄師濮嬫枃鏈紙瀛楀箷鏄剧ず锛?
    ...
    def handle_opus(self, opus_data: bytes):
        logger.bind(tag=TAG).debug(f"鎺ㄩ€佹暟鎹埌闃熷垪閲岄潰甯ф暟锝烇綖 {len(opus_data)}")
    ...
    def to_tts_stream(self, text, opus_handler: Callable[[bytes], None] = None) -> None:
        ...
        if self.delete_audio_file:
            ...
            while max_repeat_time > 0:
                try:
                    ...
                    logger.bind(tag=TAG).warning(
                        f"璇煶鐢熸垚澶辫触{5 - max_repeat_time + 1}娆? {original_text}锛岄敊璇? {e}"
                    )
                    max_repeat_time -= 1
            if max_repeat_time > 0:
                logger.bind(tag=TAG).info(
                    f"璇煶鐢熸垚鎴愬姛: {original_text}锛岄噸璇晎5 - max_repeat_time}娆?
                )
            else:
                logger.bind(tag=TAG).error(
                    f"璇煶鐢熸垚澶辫触: {original_text}锛岃妫€鏌ョ綉缁滄垨鏈嶅姟鏄惁姝ｅ父"
                )
            return None
        else:
            ...
            try:
                ...
                if max_repeat_time > 0:
                    logger.bind(tag=TAG).info(
                        f"璇煶鐢熸垚鎴愬姛: {original_text}:{tmp_file}锛岄噸璇晎5 - max_repeat_time}娆?
                    )
                else:
                    logger.bind(tag=TAG).error(
                        f"璇煶鐢熸垚澶辫触: {original_text}锛岃妫€鏌ョ綉缁滄垨鏈嶅姟鏄惁姝ｅ父"
    ...
    async def open_audio_channels(self, conn):
        ...
        # 鏍规嵁conn鐨剆ample_rate鍒涘缓缂栫爜鍣紝濡傛灉瀛愮被宸茬粡鍒涘缓鍒欎笉瑕嗙洊锛圛ndexTTS鎺ュ彛杩斿洖涓?4kHZ-寰呴噸閲囨牱澶勭悊锛?
    ...
    def store_tts_text(self, sentence_id, text):
        """瀛樺偍鎸囧畾 sentence_id 瀵瑰簲鐨勬枃鏈紝鐢ㄤ簬娴佸紡TTS鑾峰彇姝ｇ‘鐨勫瓧骞曟枃鏈?
        ...
        while not self.conn.stop_event.is_set():
            try:
                ...
                if self.conn.client_abort:
                    logger.bind(tag=TAG).info("鏀跺埌鎵撴柇淇℃伅锛岀粓姝TS鏂囨湰澶勭悊绾跨▼")
                    continue
                # 杩囨护鏃ф秷鎭細妫€鏌entence_id鏄惁鍖归厤
    ...
    def _audio_play_priority_thread(self):
        ...
        while not self.conn.stop_event.is_set():
            ...
            try:
                ...
                # 鏀跺埌涓嬩竴涓枃鏈紑濮嬫垨浼氳瘽缁撴潫鏃惰繘琛屼笂鎶?
                if sentence_type is not SentenceType.MIDDLE:
                    if self.report_on_last:
                        # 绱Н妯″紡锛氶€傜敤浜庡叏绋嬪彧鏈変竴涓闊虫祦鐨凾TS锛堝seed-tts-2.0锛?
                        # FIRST鏃跺彧璁板綍鏂囨湰锛岄煶棰戞寔缁疮绉紝浠呭湪LAST鏃剁粺涓€涓婃姤
                        if text:
                            enqueue_text = text
    ...
    def _get_segment_text(self):
        # 鍚堝苟褰撳墠鍏ㄩ儴鏂囨湰骞跺鐞嗘湭鍒嗗壊閮ㄥ垎
        full_text = "".join(self.tts_text_buff)
        current_text = full_text[self.processed_chars :]  # 浠庢湭澶勭悊鐨勪綅缃紑濮?
        last_punct_pos = -1

        # 鏍规嵁鏄惁鏄涓€鍙ヨ瘽閫夋嫨涓嶅悓鐨勬爣鐐圭鍙烽泦鍚?
        punctuations_to_use = (
            self.first_sentence_punctuations
        ...
        if last_punct_pos != -1:
            ...
            self.processed_chars += len(segment_text_raw)  # 鏇存柊宸插鐞嗗瓧绗︿綅缃?

            # 濡傛灉鏄涓€鍙ヨ瘽锛屽湪鎵惧埌绗竴涓€楀彿鍚庯紝灏嗘爣蹇楄缃负False
            if self.is_first_sentence:
    ...
    def _match_stream_text(self, text):
        """娴佸紡鏂囨湰婊戝姩绐楀彛鍖归厤锛岀敤浜庡鐞嗚法鍒嗙墖鐨勬浛鎹㈣瘝

        Args:
            text: 杈撳叆鐨勬枃鏈墖娈?

        Returns:
            tuple: (纭畾鐨勬枃鏈垪琛? 鍓╀綑寰呭尮閰嶇殑鍓嶇紑)
        """
